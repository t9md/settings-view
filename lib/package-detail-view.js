...
export default class PackageDetailView {
  constructor (pack, settingsView, packageManager, snippetsProvider) {
    ...
    const onClick = (refName, handlerBody) => {
      ...
      this.disposables.add(new Disposable(() => element.removeEventListener('click', handler)))
    }
    ...
    onClick('issueButton', () => {
      const repoUrl = this.packageManager.getRepositoryUrl(this.pack)
      if (repoUrl) shell.openExternal(`${repoUrl}/issues/new`)
    })
    onClick('changelogButton', () => {
      if (this.changelogPath) this.openMarkdownFile(this.changelogPath)
    })
    onClick('licenseButton', () => {
      if (this.licensePath) this.openMarkdownFile(this.licensePath)
    })
    onClick('openButton', () => {
      if (fs.existsSync(this.pack.path)) atom.open({pathsToOpen: [this.pack.path]})
    })
  ...
  completeInitialization () {
    if (this.refs.packageCard) {
      ...
      this.packageCard = new PackageCard(this.pack.metadata, this.settingsView, this.packageManager, {
        onSettingsView: true
      })
  ...
  show () {
    this.setVisible(this.element, true)
  }
  ...
  render () {
    ...
    if (this.pack && this.pack.metadata && this.pack.metadata.owner) {
      ...
      packageCardView = (
        <div ref='packageCardParent' className='row'>
          <div ref='loadingMessage' className='alert alert-info icon icon-hourglass'>{`Loading ${
            this.pack.name
          }\u2026`}</div>
  ...
  populate () {
    ...
    if (repoUrl) {
      ...
      this.setVisible(this.refs.packageRepo, true)
    } else {
      this.setVisible(this.refs.packageRepo, false)
    }
  ...
  updateInstalledState () {
    ...
    this.updateFileButtons()
    this.activateConfig()
    this.setVisible(this.refs.startupTime, false)
    ...
    if (atom.packages.isPackageLoaded(this.pack.name)) {
      if (!atom.packages.isPackageDisabled(this.pack.name)) {
        ...
        this.refs.startupTime.innerHTML = `This ${this.type} added <span class='highlight'>${this.getStartupTime()}ms</span> to startup time.` // prettier-ignore
        this.setVisible(this.refs.startupTime, true)
      }
    }

    const sourceCodeIsAvailable =
      this.packageManager.isPackageInstalled(this.pack.name) && !atom.packages.isBundledPackage(this.pack.name)
    this.setVisible(this.refs.openButton, sourceCodeIsAvailable)
    this.renderReadme()
  }
  ...
  updateFileButtons () {
    ...
    this.setVisible(this.refs.changelogButton, this.changelogPath)
    this.setVisible(this.refs.licenseButton, this.licensePath)
  }
