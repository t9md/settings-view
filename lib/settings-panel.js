...
export default class SettingsPanel extends CollapsibleSectionPanel {
  ...
  bindEditors () {
    const disposables = Array.from(this.element.querySelectorAll('atom-text-editor')).map(editorElement => {
      ...
      const focusHandler = () => {
        if (this.isDefault(name)) {
          const text = this.valueToString(this.getDefault(name))
  ...
  bindTooltips () {
    const disposables = Array.from(this.element.querySelectorAll('input[id], select[id], atom-text-editor[id]')).map(
      element => {
...
function elementForSetting (namespace, name, value) {
  ...
  if (namespace === 'editor') {
    ...
    if (
      ['commentStart', 'commentEnd', 'increaseIndentPattern', 'decreaseIndentPattern', 'foldEndPattern'].includes(name)
    ) {
      return document.createDocumentFragment()
    }
  }
