let SettingsView, settingsView, statusView, packageManager
...
module.exports = {
  handleURI (parsed) {
    if (parsed.pathname === '/show-package') {
      atom.workspace.open(`atom://config/packages/${parsed.query.package}`)
    }
  },
  ...
  createSettingsView (params) {
    if (!SettingsView) SettingsView = require('./settings-view')
