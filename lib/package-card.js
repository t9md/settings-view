      ...
      <div className='package-card col-lg-8'>
        ...
        <div className='meta'>
          <div ref='metaUserContainer' className='meta-user'>
            ...
            <a ref='loginLink' className='author' href={`https://atom.io/users/${owner}`}>
              {owner}
            </a>
          </div>
          <div className='meta-controls'>
            <div className='btn-toolbar'>
              ...
              <div ref='packageActionButtonGroup' className='btn-group'>
                ...
                <button type='button' className='btn icon icon-trashcan uninstall-button' ref='uninstallButton'>
                  Uninstall
                </button>
    ...
    this.packageManager.loadCompatiblePackageVersion(this.pack.name, (err, pack) => {
      ...
      if (packageVersion) {
        ...
        this.refs.packageMessage.insertAdjacentText(
          'beforeend',
          `There's no version of this package that is compatible with your Atom version. The version must satisfy ${
            this.pack.engines.atom
          }.`
        )
  ...
  displayDeprecatedState () {
    ...
    if (info && info.hasDeprecations) {
      ...
      if (isInstalled && this.packageManager.isPackageInstalled(alt)) {
        message = `\`${
          this.pack.name
        }\` has been replaced by \`${alt}\` which is already installed. Please uninstall this package.`
        this.refs.settingsButton.remove()
        this.refs.enablementButton.remove()
      } else if (isInstalled) {
    ...
    if (newVersion) {
      ...
      if (this.isInstalled()) {
        return info.message != null
          ? info.message
          : 'This package has not been loaded due to using deprecated APIs. There is no update available.'
      } else {
  ...
  */
  ...
  installAlternative () {
    ...
    this.packageManager.installAlternative(this.pack, metadata.alternative, (error, {pack, alternative}) => {
      if (error != null) {
        console.error(
          `Installing alternative \`${alternative}\` ${this.type} for ${this.pack.name} failed`,
          error.stack != null ? error.stack : error,
          error.stderr
        )
      }
